...
from .discover import get_dependency_ordered


@patch(__name__ + '.build.os.environ', {
...
class TestDependencies(unittest.TestCase):
    ...
    def test_include_downstream_dependencies(self):
        dep_ordered = get_dependency_ordered([Component.all['ployst-api']])
        self.assertEqual(
            [d.title for d in dep_ordered],
            ['ployst-api', 'integration-tests']
        )

    def test_downstream_dependencies_at_the_end(self):
        dep_ordered = get_dependency_ordered(
            [Component.all['integration-tests'],
             Component.all['ployst-api']]
        )
        self.assertEqual(
            [d.title for d in dep_ordered],
            ['ployst-api', 'integration-tests']
    ...
    def test_get_upstream_builds(self):
        upstream = Component.all['integration-tests'].get_upstream_builds()
        self.assertEqual(
            sorted([d.title for d in upstream]),
            ['ployst-api', 'ployst-ui']
        )
